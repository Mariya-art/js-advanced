...
class GoodsList {
  constructor() {
    ...
    this.cartCounter = 0; //счетчик позиций в корзине
    //без промисов
  ...
  onFetchSuccess(data) {
    ...
    this.render();
    this.buttonHandler(cartList.addToCart);
  }
  //без промисов
  /*onFetchError() {
  ...
  deletePreviousTotal() { //удаляем предыдущие расчеты стоимости корзины (если они есть)
    let masTotal = document.querySelectorAll(".total");
...
class CartItem extends GoodsItem {
  ...
  getHtml() { // переопределяем HTML-код для товара в корзине (убираем фото, добавляем кол-во и стоимость, задаем стили)
    return `<li>
      <button class="cart__cross-button">
...
class CartList {
  ...
  addToCart(e) {
    let j = goodsList.goods.findIndex(function(entry){return entry.id == e.target.id;}); //находим индекс товара с нужным нам id
    let cartId = goodsList.goods[j].id; 
    let cartLink = goodsList.goods[j].link;
    let cartTitle = goodsList.goods[j].title;
    let cartDesc = goodsList.goods[j].desc;
    let cartPrice = goodsList.goods[j].price;
    let cartItem = new CartItem(cartId, cartLink, cartTitle, cartDesc, cartPrice); //создаем товар в корзине
    this.$cartList = document.querySelector('.cart__list'); //место куда добавим корзину
    this.$cartList.insertAdjacentHTML("beforeEnd", cartItem.getHtml());//добавляем товар в корзину
    goodsList.cartCounter += 1; //счетчик позиций в корзине
    document.querySelector(".header__cart-circle").innerText = goodsList.cartCounter; //добавляем счетчик
    document.querySelector(".cart__text").style.display="none"; //убираем текст "Ваша корзина пуста"
    goodsList.total += cartItem.value; //рассчитываем стоимость корзины
    this.$cartList.insertAdjacentHTML("afterEnd", `<div class="total"><p class="total__text">Grand total of shopping cart<span class="colortext text-margin text-bold">$${goodsList.total}</span></p></div>`); //добавляем сумму корзины
    goodsList.deletePreviousTotal(); //удаляем предыдущие расчеты стоимости корзины (если они есть)
  }
...
class Header {
  constructor() {
    this.$container = document.querySelector('.header__list');
...
header.cartButtonHandler(cartList.openCart);